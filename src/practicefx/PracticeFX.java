...
public class PracticeFX extends Application implements EventHandler<ActionEvent> {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        Button btnTestTreeView = new Button("Show new window implementing Tree View");
        btnTestTreeView.setOnAction(e -> {
            window.hide();
            if (TestTreeView.display()) {
                window.show();
            }
        });
        
        
        Button btnTestTableView = new Button("Show new window implementing Table View");
        btnTestTableView.setOnAction(e -> {
            window.hide();
            if (TestTableView.display()) {
                window.show();
            }
        });
...
//        primaryLayout.getChildren().addAll(label, btnAnon, btnLambda, btnLambdaMultiline);
        ...
        scenePrimary = new Scene(primaryLayout, 400, 600);
